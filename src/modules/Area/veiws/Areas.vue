                ...
                <vue3-datatable :rows="rows" :columns="cols" :totalRows="rows?.length" :sortable="true" :search="search"
                    ...
                    <template #action="data">
                        <div class="flex items-center gap-2">
                            <div>
                                <button @click="editArea(data)" type="button" v-tippy:Edit>
                                    <iconEdit />
                                </button>
                                <tippy target="Edit">Edit</tippy>
                            </div>
                            <div>
                                <button @click="deleteArea(data)" type="button" v-tippy:Delete>
                                    <iconTrash />
                                </button>
...
import { AllAreas, DeleteArea } from "../composables/useArea";
...
const deleteArea = async (data) => {
    ...
    if (isConfirmed) {
        ...
        try {
            ...
            await DeleteArea(id);
            showMessage("Area deleted !", "success");
        } catch (error) {
            console.log(error);
...
const editArea = async (data: any) => {
    router.push({ name: 'update-area', params: { id: data.value._id } });
};

const fetchAreas = async () => {
    try {
        isLoading.value = true;
        let response: any = await AllAreas();
        rows.value = response.areas;
        isLoading.value = false;
    } catch (error) {
        console.log(error);
    }
