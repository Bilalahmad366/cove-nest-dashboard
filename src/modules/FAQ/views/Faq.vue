        ...
        >
          <template #action="data">
            <div class="flex items-center gap-2">
              <div>
                <button @click="editFaq(data)" type="button" v-tippy:Edit>
                  <iconEdit />
                </button>
                <tippy target="Edit">Edit</tippy>
              </div>
              <div>
                <button @click="deleteFaq(data)" type="button" v-tippy:Delete>
                  <iconTrash />
                </button>
...
import { AllFaq, DeleteFaq } from "../../videos/composables/useVideos";
...
const cols = ref([
  { field: "question", title: "Question", visible: true, hide: false, disabled: true },
  { field: "answer", title: "Answer", visible: true, hide: false, disabled: true },
...
const editFaq = (data: any) => {
  router.push({ name: "update-faq", params: { id: data.value._id } })
}

const fetchFaqs = async () => {
  try {
    isLoading.value = true
    let response: any = await AllFaq()
    rows.value = response.flatMap((faq: any) =>
      faq.faqs?.map((f: any) => ({
        _id: faq._id,
        question: f.question,
        answer: f.answer,
