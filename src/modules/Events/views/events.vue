                ...
                <vue3-datatable :rows="rows" :columns="cols" :totalRows="rows?.length" :sortable="true" :search="search"
                    ...
                    <template #action="data">
                        <div class="flex items-center gap-2">
                            <div>
                                <button @click="editEvents(data)" type="button" v-tippy:Edit>
                                    <iconEdit />
                                </button>
                                <tippy target="Edit">Edit</tippy>
                            </div>
                            <div>
                                <button @click="deleteEvents(data)" type="button" v-tippy:Delete>
                                    <iconTrash />
                                </button>
...
import { AllEvents, DeleteEvent } from "../composables/useEvents";
...
const deleteEvents = async (data) => {
    ...
    if (isConfirmed) {
        ...
        try {
            ...
            showMessage("Events deleted !", "success");
        } catch (error) {
            console.log(error);
...
const editEvents = async (data: any) => {
    router.push({ name: 'update-event', params: { id: data.value._id } });
};

const fetchEvents = async () => {
    try {
        isLoading.value = true;
        let response: any = await AllEvents();
        rows.value = response;
        isLoading.value = false;
    } catch (error) {
        console.log(error);
